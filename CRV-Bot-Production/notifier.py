...
def notify_order_opened(side: str, price: float, size_usdt: float, paper: bool) -> None:
    ...
    _send(
        ...
        f"Stop Loss:   <code>-15% from entry</code>\n"
...
def notify_circuit_breaker(peak_equity: float, current_equity: float, drawdown_pct: float) -> None:
    _send(
        f"⛔ <b>CIRCUIT BREAKER TRIGGERED — BOT HALTED</b>\n\n"
        ...
        f"⚠️ Bot will NOT trade until manually restarted.\n"
        f"To reset: run <code>python bot_runner.py --reset-cb</code>\n"
        f"Time: {_now()}"
    )
