...
#   Stop Loss    : -15% dari entry price
#   Circuit Breaker: -35% dari all-time peak equity -> halt
#   Leverage     : 2x
#   Position Size: 90% equity per trade (notional = equity * 0.9 * 2)
#   Timeframe    : 4H
#   Modal        : $10,000 USDT
#
...
STOP_LOSS_PCT      = 0.15       # -15% dari entry price
CIRCUIT_BREAKER_DD = 0.35       # -35% dari peak equity
RSI_PERIOD         = 14
RSI_OVERSOLD       = 30
RSI_OVERBOUGHT     = 70
BB_PERIOD          = 20
...
def run_backtest(df: pd.DataFrame):
    ...
    for i in range(len(df)):
        ...
        # -- Circuit Breaker (already hit -> stay halted) --
        if circuit_breaker:
            continue
        ...
        if dd_from_peak >= CIRCUIT_BREAKER_DD:
            ...
            print(f"  [!] Circuit Breaker @ {ts.strftime('%Y-%m-%d %H:%M')} | "
                  f"Equity ${equity:.2f} | DD {dd_from_peak*100:.1f}%")
            continue
...
def plot_backtest(df, trades, stats, sig_l, sig_s, sig_sl, sig_cb):
    ...
    gs = gridspec.GridSpec(
        5, 2, figure=fig,
        height_ratios=[3, 1.2, 1.5, 1.5, 1.5],
    ...
    ax_price.fill_between(dates, df["bb_upper"], df["bb_lower"],
                          alpha=0.08, color=ACCENT_BLUE, label="BB Band (20,2)")
    ...
    fig.text(
        ...
        f"Strategy: RSI(14)+BB(20,2s) 4H  |  Leverage: {LEVERAGE}x  |  "
